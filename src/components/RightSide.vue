...
const activeBlock = ref(about.blocks[0].slug);
</script>
...
<template>
  <div class="space-y-5 lg:col-span-2">
    ...
    <div v-for="item in about[activeBlock]" v-show="item.is === activeBlock" :key="item.titleBlock" class="p-7 block-section">
      <h2 class="block-title">{{ item.titleBlock }}</h2>
      <div class="mb-5 item-section" v-for="(block, i) in item.blocks" :key="block.title">
        <div class="flex-shrink-0 w-12 h-12 rounded-xl bg-cover" :style="`background-image: url('${block.logoUrl}');`" />
        <div class="w-full space-y-5">
          <div class="item-header items-end">
            ...
            <div v-if="block.period" class="space-y-2 sm:text-right">
              <div v-if="block.timeJob" class="job-item-badge">{{ block.timeJob }}</div>
              <div class="item-header-info">
          ...
          <button v-if="block.link"
            class="flex items-center space-x-3 px-3.5 py-1.5 rounded-lg group border border-purple-500 bg-white text-purple-500 text-sm font-medium transition duration-200 hover:border-purple-600 hover:text-white hover:bg-purple-600">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
